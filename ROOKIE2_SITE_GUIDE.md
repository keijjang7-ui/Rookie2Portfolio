...
`Hero`와 `Intro`는 `.hero-intro` 래퍼 안에 함께 들어 있습니다. Hero 배경은 별도 fixed 레이어로 두고, Hero 텍스트와 Intro 섹션이 스크롤 흐름 안에서 움직입니다.
...
Hero 배경은 완전 고정이 아니라 약한 parallax로 움직입니다.

- `.hero-intro__background`는 `position: fixed`입니다.
...
- 560 이하 모바일과 1200 이하 터치 환경에서는 현재 기준보다 약 `1.5배` 빠른 스크럽을 위해 `233.333svh`, 최소 `1320px`
...
- 위치는 모든 반응형 지점에서 공통 정보 영역 `--section-width`의 왼쪽 기준선과 맞춥니다. 1920 기준 `x: 360`, 1600 기준 `x: 200`, 1200 기준 `x: 40`, 모바일 390 기준 `x: 24`입니다.
- 최대 폭: `600px`
...
- Hero 배경은 fixed + JS parallax입니다. 단순 sticky로 바꾸면 이전처럼 의도대로 동작하지 않을 수 있습니다.
