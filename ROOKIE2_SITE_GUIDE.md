...
- `assets/images/system/system-info2-card-01.webp`: System `System_info2` 1번 슬라이드의 왼쪽 큰 이미지 카드입니다. 사용자 제공 `image 3.png`를 원본 해상도 `1920 x 1080` 그대로 WebP로 변환했고, 최종 파일은 알파 채널이 없는 RGB WebP입니다.
- `assets/images/system/system-info2-card-02.webp`, `assets/images/system/system-info2-card-03.webp`: System `System_info2` 1번 슬라이드의 오른쪽 2개 이미지 카드입니다. Figma 카드 노드를 PNG로 가져온 뒤 WebP로 변환해 최종 자산으로 보관합니다.
- `assets/images/system/system-info2-slide-02.webp`, `assets/images/system/system-info2-slide-03-left.webp`, `assets/images/system/system-info2-slide-03-right.webp`: System `System_info2` 캐러셀의 2, 3번째 슬라이드 이미지입니다. `system-info2-slide-03-left.webp`는 사용자 제공 `videoframe_564.png`를 원본 해상도 `720 x 1090` 그대로 WebP로 변환했고, 최종 파일은 알파 채널이 없는 RGB WebP입니다.
...
- `System_info2` 카드 영역은 `1200 x 675` 스틸 이미지 캐러셀입니다. 1번 슬라이드는 왼쪽 `586 x 675` 큰 이미지 카드(`system-info2-card-01.webp`)와 오른쪽 `586 x 323.5` 이미지 카드 2개(`system-info2-card-02.webp`, `system-info2-card-03.webp`)를 `28px` gap으로 쌓습니다. 2번 슬라이드는 단일 이미지 `system-info2-slide-02.webp`입니다. 3번 슬라이드는 왼쪽 `system-info2-slide-03-left.webp`, 오른쪽 `system-info2-slide-03-right.webp` 2분할 이미지입니다. 데스크톱 radius는 `36px`입니다.
...
- `initializeSystemInfo2Carousel()`: `System_info2`의 3개 슬라이드와 인디케이터를 초기화합니다. Visual Principles와 같은 drag/wheel/indicator snap 방식을 사용합니다.
